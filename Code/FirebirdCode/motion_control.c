...
#include "lcd.c" 		// Contains various functions to manipulate the LCD on firebird

#define FCPU 11059200ul // Defined here to make sure that program works properly

unsigned char data; 	// To store received data from UDR3
...
/*
	Receive Interrupt Handler. Puts the value received from bluetooth on UDR3 in variable data.
*/
...
int main(void)
{
   	...
   	init_devices();
   	Init_USART3(); 	// Initialize the USART3 for bluetooth communication
	lcd_set_4bit(); // Set lcd in 4-bit mode
	lcd_init(); 	// Initialize LCD

	int flag_buzzer_on = 0; // For toggling buzzer. Initially the buzzer is off	

	unsigned int signal; 

	velocity(120,120); // Set firebird velocity to necessary value

	while(1) {
		
		signal = (unsigned int)data; // data contains the character we receive from the Bluetooth module on UDR3
		
		
		/* 
			Based on what value we get in data, take appropriate action
			8 - Move Forward
			2 - Move Backward
			4 - Take a soft left turn
			6 - Take a soft right turn
			7 - Toggle Buzzer
		*/
		switch(signal) {  
			case 8:
			...
			case 7:
				// Toggle the buzzer on receiving 7
				if (flag_buzzer_on == 0){
					buzzer_on();
					flag_buzzer_on = 1;
				} 
